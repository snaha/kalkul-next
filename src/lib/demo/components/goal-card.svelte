<script lang="ts">
	...
	const linkedInvestments = $derived(
		goal?.linkedInvestments.map((li) => ({
			investment: demoStore.investments.find((inv) => inv.id === li.investmentId),
		...
		{:else}
			<div class="dropdown">
				<Dropdown left buttonDimension="compact">
					...
					{/snippet}
					<List>
						<ListItem onclick={() => notImplemented($locale)}
	...
	<div class="trasaction-container" class:modalShow={!open || hidden}>
				...
				<Horizontal --horizontal-gap="var(--quarter-padding)">
					...
					<Button
						...
						onclick={(e: Event) => {
							e.preventDefault()
							e.stopPropagation()
							const goalIndex = goal ? demoStore.goals.findIndex((g) => g === goal) : 0
							goto(routes.DEMO_BALANCE_INVESTMENTS(goalIndex))
						}}
					>
		...
		{#if !viewOnly && addInvestment && linkedInvestments.length === 0}
			<Vertical --vertical-gap="var(--quarter-padding)">
				...
				<Typography variant="small" class="add-investments-explanation">
...
<style type="postcss">
	...
	:global(.add-investments-explanation) {
